...
fn python_label(fixture: &str) -> String {
    let out = Command::new("python3")
        .arg(concat!(env!("CARGO_MANIFEST_DIR"), "/../../tools/canonical-gguf.py"))
        .arg(fixture)
        .output()
        .expect("run canonical-gguf.py");
    assert!(out.status.success(), "python tool failed: {}", String::from_utf8_lossy(&out.stderr));
...
fn python_matches_rust_for_fixtures() {
    ...
    for name in ["synthetic-f32.gguf", "empty-metadata.gguf", "aligned-256.gguf"] {
        let path = format!("{dir}/{name}");
        let bytes = std::fs::read(&path).unwrap();
        let rust = uor_addr::gguf::address(&bytes).unwrap().address.as_str().to_string();
