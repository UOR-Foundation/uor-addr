//! `gguf::address` — the ONNX realization's public entry point. Mirrors
//! [`crate::ring::pipeline::address`] for `V = OnnxValue`: parse at the
...
pub enum AddressFailure {
    /// The input bytes are not a well-formed ONNX ModelProto (bad magic,
    ...
    /// exceeded under the active [`OnnxAddrBounds`] profile.
    TooLarge,
...
pub fn address(input_bytes: &[u8]) -> Result<AddressOutcome, AddressFailure> {
    let value = OnnxValue::parse(input_bytes).map_err(|v| {
        ...
        if c.ends_with("/withinBounds")
            || c.ends_with("/subgraphDepthBound")
