...
//! k-invariants branch ψ_1 → ψ_7 → ψ_8 → ψ_9 — over `Input = OnnxValue`.
...
verb! {
    pub fn address_inference(input: OnnxValue) -> AddressLabel {
...
mod tests {
    ...
    fn verb_arena_contains_no_sigma_residuals() {
        ...
        assert!(!arena.iter().any(|t| matches!(t, Term::AxisInvocation { .. })));
    }
}
