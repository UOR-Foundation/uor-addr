//! ONNX realization's `ResolverTuple` — the eight ψ-stage resolvers.
//!
//! Structurally identical to [`crate::ring::resolvers`]: the ψ_1 → ψ_7 →
//! ψ_8 chain threads the canonical structural bytes through a fixed
//! carrier; ψ_9 dispatches [`crate::common::AddressInput::canonicalize_into`]
//! on [`crate::onnx::OnnxValue`] inside its resolver body (ADR-046) and
//! projects through the canonical hash axis to mint the κ-label. Only the
//! carrier width and the typed-input type differ from `ring`.
...
use crate::onnx::shapes::bounds::{OnnxAddrBounds, ONNX_CANON_MAX_BYTES};
...
const CARRIER_BYTES: usize = <OnnxAddrBounds as HostBounds>::TERM_VALUE_MAX_BYTES;
...
const _: () = {
    assert!(
        ONNX_CANON_MAX_BYTES <= CARRIER_PAYLOAD_MAX,
        "ONNX_CANON_MAX_BYTES must fit within the ψ-carrier payload region"
    );
};
...
macro_rules! gguf_violation {
    ($name:ident, $iri:literal, $min:expr, $max:expr) => {
        const $name: ShapeViolation = ShapeViolation {
            shape_iri: concat!("https://uor.foundation/addr/onnx/resolver/", $iri),
            constraint_iri: concat!("https://uor.foundation/addr/onnx/resolver/", $iri, "/bound"),
            property_iri: concat!("https://uor.foundation/addr/onnx/resolver/", $iri, "/byteCount"),
...
gguf_violation!(INPUT_LEN_VIOLATION, "InputBytes", 0, ONNX_CANON_MAX_BYTES as u32);
gguf_violation!(
    CARRIER_INPUT_VIOLATION,
    "CarrierInput",
    CARRIER_BYTES as u32,
    CARRIER_BYTES as u32
);
gguf_violation!(CARRIER_BUFFER_VIOLATION, "CarrierBuffer", CARRIER_BYTES as u32, 0);
gguf_violation!(
    ADDR_OUTPUT_VIOLATION,
    "AddressLabelBuffer",
    WIRE_FORMAT_ADDRESS_BYTES as u32,
    0
);
gguf_violation!(UPSTREAM_TAG_VIOLATION, "UpstreamStageTag", 1, 1);
gguf_violation!(
    GEOMETRY_VIOLATION,
    "CarrierGeometry",
    NERVE_VERTEX_COUNT,
...
fn decode_carrier(bytes: &[u8]) -> Result<DecodedCarrier<'_>, ShapeViolation> {
    ...
    if canonical_len > ONNX_CANON_MAX_BYTES {
...
fn validate_upstream(
    ...
    if carrier.vertex_count != NERVE_VERTEX_COUNT || carrier.highest_dim != NERVE_HIGHEST_DIM {
...
fn emit_carrier(canonical: &[u8], stage_tag: u64, out: &mut [u8]) -> Result<usize, ShapeViolation> {
    ...
    if canonical.len() > ONNX_CANON_MAX_BYTES {
        return Err(INPUT_LEN_VIOLATION);
    }
    ...
    out[LENGTH_PREFIX_BYTES + canonical.len()..OFFSET_GEOMETRY_TAIL].fill(0);
...
impl<H: Hasher> NerveResolver<H> for AddressNerveResolver<H> {
    ...
    fn resolve(&self, input: &[u8], out: &mut [u8]) -> Result<usize, ShapeViolation> {
        if input.len() > ONNX_CANON_MAX_BYTES {
            return Err(INPUT_LEN_VIOLATION);
        }
...
impl<H: Hasher> ChainComplexResolver<H> for AddressChainComplexResolver<H> {
    ...
    fn resolve(&self, input: SimplicialComplexBytes<'_>, out: &mut [u8]) -> Result<usize, ShapeViolation> {
        let c = decode_carrier(input.as_bytes())?;
        validate_upstream(&c, PSI_1_NERVE_TAG)?;
        emit_carrier(c.canonical, PSI_2_CHAIN_COMPLEX_TAG, out)
    }
}

impl<H: Hasher> HomologyGroupResolver<H> for AddressHomologyGroupResolver<H> {
    #[inline]
    fn resolve(&self, input: ChainComplexBytes<'_>, out: &mut [u8]) -> Result<usize, ShapeViolation> {
        let c = decode_carrier(input.as_bytes())?;
        validate_upstream(&c, PSI_2_CHAIN_COMPLEX_TAG)?;
        emit_carrier(c.canonical, PSI_3_HOMOLOGY_GROUPS_TAG, out)
    }
}
...
impl<H: Hasher> CochainComplexResolver<H> for AddressCochainComplexResolver<H> {
    ...
    fn resolve(&self, input: ChainComplexBytes<'_>, out: &mut [u8]) -> Result<usize, ShapeViolation> {
        let c = decode_carrier(input.as_bytes())?;
        validate_upstream(&c, PSI_2_CHAIN_COMPLEX_TAG)?;
        emit_carrier(c.canonical, PSI_5_COCHAIN_COMPLEX_TAG, out)
    }
}

impl<H: Hasher> CohomologyGroupResolver<H> for AddressCohomologyGroupResolver<H> {
    #[inline]
    fn resolve(&self, input: CochainComplexBytes<'_>, out: &mut [u8]) -> Result<usize, ShapeViolation> {
        let c = decode_carrier(input.as_bytes())?;
        validate_upstream(&c, PSI_5_COCHAIN_COMPLEX_TAG)?;
        emit_carrier(c.canonical, PSI_6_COHOMOLOGY_GROUPS_TAG, out)
    }
}
...
impl<H: Hasher> PostnikovResolver<H> for AddressPostnikovResolver<H> {
    ...
    fn resolve(&self, input: SimplicialComplexBytes<'_>, out: &mut [u8]) -> Result<usize, ShapeViolation> {
        let c = decode_carrier(input.as_bytes())?;
        validate_upstream(&c, PSI_1_NERVE_TAG)?;
        emit_carrier(c.canonical, PSI_7_POSTNIKOV_TOWER_TAG, out)
    }
}

impl<H: Hasher> HomotopyGroupResolver<H> for AddressHomotopyGroupResolver<H> {
    #[inline]
    fn resolve(&self, input: PostnikovTowerBytes<'_>, out: &mut [u8]) -> Result<usize, ShapeViolation> {
        let c = decode_carrier(input.as_bytes())?;
        validate_upstream(&c, PSI_7_POSTNIKOV_TOWER_TAG)?;
        emit_carrier(c.canonical, PSI_8_HOMOTOPY_GROUPS_TAG, out)
    }
}

impl<H: Hasher> KInvariantResolver<H> for AddressKInvariantResolver<H> {
    #[inline]
    fn resolve(&self, input: HomotopyGroupsBytes<'_>, out: &mut [u8]) -> Result<usize, ShapeViolation> {
        if out.len() < WIRE_FORMAT_ADDRESS_BYTES {
            return Err(ADDR_OUTPUT_VIOLATION);
        }
        let c = decode_carrier(input.as_bytes())?;
        validate_upstream(&c, PSI_8_HOMOTOPY_GROUPS_TAG)?;

        let mut canonical = [0u8; ONNX_CANON_MAX_BYTES];
        let n = <OnnxValue as AddressInput>::canonicalize_into(c.canonical, &mut canonical)?;

        let digest: [u8; 32] = H::initial().fold_bytes(&canonical[..n]).finalize_to_32();
...
const _: () = {
    assert!(CARRIER_BYTES <= <OnnxAddrBounds as HostBounds>::NERVE_OUTPUT_BYTES_MAX);
    assert!(CARRIER_BYTES <= <OnnxAddrBounds as HostBounds>::POSTNIKOV_TOWER_OUTPUT_BYTES_MAX);
    assert!(CARRIER_BYTES <= <OnnxAddrBounds as HostBounds>::HOMOTOPY_GROUPS_OUTPUT_BYTES_MAX);
    assert!(WIRE_FORMAT_ADDRESS_BYTES <= <OnnxAddrBounds as HostBounds>::K_INVARIANTS_OUTPUT_BYTES_MAX);
};
