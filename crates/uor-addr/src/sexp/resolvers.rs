//! S-expression realization's `ResolverTuple` — the eight ψ-stage
...
//! [`crate::sexp::SExprValue`] inside its resolver body (ADR-046
...
use crate::sexp::shapes::bounds::{SExprAddrBounds, SEXPR_VALUE_MAX_BYTES};
...
const CARRIER_BYTES: usize = <SExprAddrBounds as HostBounds>::TERM_VALUE_MAX_BYTES;
...
const _: () = {
    assert!(
        SEXPR_VALUE_MAX_BYTES <= CARRIER_PAYLOAD_MAX,
        "SEXPR_VALUE_MAX_BYTES must fit within CARRIER_BYTES - LENGTH_PREFIX_BYTES - GEOMETRY_TAIL_BYTES"
    );
};
...
const INPUT_LEN_VIOLATION: ShapeViolation = ShapeViolation {
    shape_iri: "https://uor.foundation/addr/sexp/resolver/SExprValueBytes",
    constraint_iri: "https://uor.foundation/addr/sexp/resolver/SExprValueBytes/maxBytes",
    property_iri: "https://uor.foundation/addr/sexp/resolver/SExprValueBytes/byteCount",
...
const CARRIER_INPUT_VIOLATION: ShapeViolation = ShapeViolation {
    shape_iri: "https://uor.foundation/addr/sexp/resolver/CarrierInput",
    constraint_iri: "https://uor.foundation/addr/sexp/resolver/CarrierInput/expectedCarrierBytes",
    property_iri: "https://uor.foundation/addr/sexp/resolver/CarrierInput/byteCount",
...
const CARRIER_BUFFER_VIOLATION: ShapeViolation = ShapeViolation {
    shape_iri: "https://uor.foundation/addr/sexp/resolver/CarrierBuffer",
    constraint_iri: "https://uor.foundation/addr/sexp/resolver/CarrierBuffer/minBytes",
    property_iri: "https://uor.foundation/addr/sexp/resolver/CarrierBuffer/byteCount",
    expected_range: "http://www.w3.org/2001/XMLSchema#nonNegativeInteger",
    min_count: CARRIER_BYTES as u32,
    max_count: 0,
    kind: ViolationKind::ValueCheck,
...
const ADDR_OUTPUT_VIOLATION: ShapeViolation = ShapeViolation {
    shape_iri: "https://uor.foundation/addr/sexp/resolver/AddressLabelBuffer",
    constraint_iri: "https://uor.foundation/addr/sexp/resolver/AddressLabelBuffer/minBytes",
    property_iri: "https://uor.foundation/addr/sexp/resolver/AddressLabelBuffer/byteCount",
    expected_range: "http://www.w3.org/2001/XMLSchema#nonNegativeInteger",
...
const UPSTREAM_TAG_VIOLATION: ShapeViolation = ShapeViolation {
    shape_iri: "https://uor.foundation/addr/sexp/resolver/UpstreamStageTag",
    constraint_iri: "https://uor.foundation/addr/sexp/resolver/UpstreamStageTag/expectedTag",
    property_iri: "https://uor.foundation/addr/sexp/resolver/UpstreamStageTag/stageTag",
...
const GEOMETRY_VIOLATION: ShapeViolation = ShapeViolation {
    shape_iri: "https://uor.foundation/addr/sexp/resolver/CarrierGeometry",
    constraint_iri:
        "https://uor.foundation/addr/sexp/resolver/CarrierGeometry/seventyOneVerticesDiscrete",
    property_iri: "https://uor.foundation/addr/sexp/resolver/CarrierGeometry/vertexCount",
    expected_range: "http://www.w3.org/2001/XMLSchema#unsignedInt",
...
fn decode_carrier(bytes: &[u8]) -> Result<DecodedCarrier<'_>, ShapeViolation> {
    ...
    if canonical_len > SEXPR_VALUE_MAX_BYTES {
...
fn emit_carrier(canonical: &[u8], stage_tag: u64, out: &mut [u8]) -> Result<usize, ShapeViolation> {
    ...
    if canonical.len() > SEXPR_VALUE_MAX_BYTES {
        return Err(INPUT_LEN_VIOLATION);
    }
...
impl<H: Hasher> NerveResolver<H> for AddressNerveResolver<H> {
    ...
    fn resolve(&self, input: &[u8], out: &mut [u8]) -> Result<usize, ShapeViolation> {
        if input.len() > SEXPR_VALUE_MAX_BYTES {
            return Err(INPUT_LEN_VIOLATION);
        }
...
impl<H: Hasher> KInvariantResolver<H> for AddressKInvariantResolver<H> {
    ...
    fn resolve(
        ...
        let mut canonical = alloc::vec![0u8; SEXPR_VALUE_MAX_BYTES];
        let canonical_len =
            <SExprValue as AddressInput>::canonicalize_into(carrier.canonical, &mut canonical)?;
...
const _: () = {
    assert!(CARRIER_BYTES <= <SExprAddrBounds as HostBounds>::NERVE_OUTPUT_BYTES_MAX);
    assert!(CARRIER_BYTES <= <SExprAddrBounds as HostBounds>::CHAIN_COMPLEX_OUTPUT_BYTES_MAX);
    assert!(CARRIER_BYTES <= <SExprAddrBounds as HostBounds>::HOMOLOGY_GROUPS_OUTPUT_BYTES_MAX);
    assert!(CARRIER_BYTES <= <SExprAddrBounds as HostBounds>::COCHAIN_COMPLEX_OUTPUT_BYTES_MAX);
    assert!(CARRIER_BYTES <= <SExprAddrBounds as HostBounds>::COHOMOLOGY_GROUPS_OUTPUT_BYTES_MAX);
    assert!(CARRIER_BYTES <= <SExprAddrBounds as HostBounds>::POSTNIKOV_TOWER_OUTPUT_BYTES_MAX);
    assert!(CARRIER_BYTES <= <SExprAddrBounds as HostBounds>::HOMOTOPY_GROUPS_OUTPUT_BYTES_MAX);
    assert!(
        WIRE_FORMAT_ADDRESS_BYTES <= <SExprAddrBounds as HostBounds>::K_INVARIANTS_OUTPUT_BYTES_MAX
    );
};
