...
//! [`crate::json::shapes::bounds`] (depth ≤ `MAX_JSON_DEPTH`, per-string
...
//! [`crate::json::shapes::bounds::JSON_VALUE_MAX_BYTES`].
...
/// Slice-output variant of [`canonicalize_into`] — the byte-output
/// signature [`crate::common::AddressInput::canonicalize_into`]
/// requires. Walks the tagged JSON bytes, emits the JCS-RFC8785 +
/// Unicode NFC canonical form into `out`, and returns the number of
/// bytes written.
///
/// # Errors
///
...
///   enough to hold the canonical form; for the JSON realization,
///   [`JSON_VALUE_MAX_BYTES`] suffices because JCS+NFC
...
// ─── AddressInput impl — the architectural-surface conformance ────────
//
// JSON is UOR-ADDR's first-published format-specific realization
// (ARCHITECTURE.md "Format-specific realizations" § uor-addr-json).
// `JsonValue` satisfies the common [`AddressInput`] trait by
// supplying:
//
// - `type Registry = EmptyShapeRegistry` — the current `JsonValue`
//   constraint geometry carries no `ConstraintRef::Recurse` entries
//   (the typed-input shape is flat under the structurally-tagged
//   byte serialization), so the application registry is empty. A
//   future migration to the `partition_coproduct!` over the seven
//   JSON cases — `JsonValueNull`, `JsonValueBoolFalse`,
//   `JsonValueBoolTrue`, `JsonValueNumber`, `JsonValueString`,
//   `JsonValueArray`, `JsonValueObject` — with the Array and Object
//   cases bearing `recurse(MAX_JSON_DEPTH):JsonValue` operands per
//   ADR-057 would populate the registry via `register_shape!`.
//
// - `parse` — host-boundary parser per SD2 Grounding stage. Wraps
//   [`JsonValue::parse`].
//
// - `canonicalize_into` — JCS-RFC8785 + Unicode NFC under
//   ADR-046's resolver-body iterative-resolution discipline. Wraps
//   [`canonicalize_into_slice`].
// ─── ADR-057 application shape registry ───────────────────────────────
//
// The `register_shape!` SDK macro emits a fresh marker type
// (`JsonValueRegistry`) whose `ShapeRegistryProvider` impl aggregates
// the realization's registered shapes. ψ_1's registry-aware
// nerve-expansion path (`primitive_simplicial_nerve_betti_in::<Self,
// Self::Registry>` / `expand_constraints_in::<Self::Registry>`)
// consults this registry at evaluation time per ADR-057.
//
// The current `JsonValue` carries no `ConstraintRef::Recurse` entries
// (the typed-input shape is flat under the structurally-tagged byte
// serialization), so the registry's `JsonValue` entry's
// `constraints` slice is empty. A future migration to the
// `partition_coproduct!` over the seven JSON cases (per
// ARCHITECTURE.md "Format-specific realizations" § `uor-addr-json`)
// would expand the registered set to include each case's shape, with
// `JsonValueArray` and `JsonValueObject` bearing
// `recurse(MAX_JSON_DEPTH):JsonValue` operands that ψ_1 expands via
// the registry.
