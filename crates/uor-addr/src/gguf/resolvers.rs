//! GGUF realization's `ResolverTuple` — the eight ψ-stage resolvers.
//!
...
//! on [`crate::gguf::GgufValue`] inside its resolver body (ADR-046) and
...
use crate::gguf::shapes::bounds::{GgufAddrBounds, GGUF_CANON_MAX_BYTES};
...
const CARRIER_BYTES: usize = <GgufAddrBounds as HostBounds>::TERM_VALUE_MAX_BYTES;
...
const _: () = {
    assert!(
        GGUF_CANON_MAX_BYTES <= CARRIER_PAYLOAD_MAX,
        "GGUF_CANON_MAX_BYTES must fit within the ψ-carrier payload region"
    );
};
...
macro_rules! gguf_violation {
    ($name:ident, $iri:literal, $min:expr, $max:expr) => {
        const $name: ShapeViolation = ShapeViolation {
            shape_iri: concat!("https://uor.foundation/addr/gguf/resolver/", $iri),
            constraint_iri: concat!("https://uor.foundation/addr/gguf/resolver/", $iri, "/bound"),
            property_iri: concat!("https://uor.foundation/addr/gguf/resolver/", $iri, "/byteCount"),
...
gguf_violation!(INPUT_LEN_VIOLATION, "InputBytes", 0, GGUF_CANON_MAX_BYTES as u32);
gguf_violation!(
...
fn decode_carrier(bytes: &[u8]) -> Result<DecodedCarrier<'_>, ShapeViolation> {
    ...
    if canonical_len > GGUF_CANON_MAX_BYTES {
...
fn emit_carrier(canonical: &[u8], stage_tag: u64, out: &mut [u8]) -> Result<usize, ShapeViolation> {
    ...
    if canonical.len() > GGUF_CANON_MAX_BYTES {
        return Err(INPUT_LEN_VIOLATION);
    }
...
impl<H: Hasher> NerveResolver<H> for AddressNerveResolver<H> {
    ...
    fn resolve(&self, input: &[u8], out: &mut [u8]) -> Result<usize, ShapeViolation> {
        if input.len() > GGUF_CANON_MAX_BYTES {
            return Err(INPUT_LEN_VIOLATION);
        }
...
impl<H: Hasher> KInvariantResolver<H> for AddressKInvariantResolver<H> {
    ...
    fn resolve(&self, input: HomotopyGroupsBytes<'_>, out: &mut [u8]) -> Result<usize, ShapeViolation> {
        ...
        let mut canonical = [0u8; GGUF_CANON_MAX_BYTES];
        let n = <GgufValue as AddressInput>::canonicalize_into(c.canonical, &mut canonical)?;
...
const _: () = {
    assert!(CARRIER_BYTES <= <GgufAddrBounds as HostBounds>::NERVE_OUTPUT_BYTES_MAX);
    assert!(CARRIER_BYTES <= <GgufAddrBounds as HostBounds>::POSTNIKOV_TOWER_OUTPUT_BYTES_MAX);
    assert!(CARRIER_BYTES <= <GgufAddrBounds as HostBounds>::HOMOTOPY_GROUPS_OUTPUT_BYTES_MAX);
    assert!(WIRE_FORMAT_ADDRESS_BYTES <= <GgufAddrBounds as HostBounds>::K_INVARIANTS_OUTPUT_BYTES_MAX);
};
