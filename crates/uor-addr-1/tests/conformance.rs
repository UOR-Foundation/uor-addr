...
/// CS-S01 — zero `unsafe` blocks anywhere in `crates/uor-addr-1/src/`.
#[test]
...
fn cs_s02__no_panic_paths_in_pipeline() {
    let sources = ["verbs.rs", "resolvers.rs", "pipeline.rs"];
    let crate_src = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src");
    for fname in sources {
        let path = crate_src.join(fname);
        let body = std::fs::read_to_string(&path)
...
/// `UorAddr1.HexEncoding.hexLower` definition byte-for-byte across
/// `[0, 16)`.
#[test]
