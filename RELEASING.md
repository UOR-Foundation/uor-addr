...
Every crate + binding ships under one shared version. The release
workflow refuses any tag that doesn't match every `version` field.
Update all six in one commit:

```text
crates/uor-addr/Cargo.toml         version = "X.Y.Z"
crates/uor-addr-c/Cargo.toml       version = "X.Y.Z"
crates/uor-addr-wasm/Cargo.toml    version = "X.Y.Z"
bindings/npm/package.json          "version": "X.Y.Z"
bindings/python/pyproject.toml     version = "X.Y.Z"
Cargo.lock                          (regenerated by `cargo update -w`)
```
