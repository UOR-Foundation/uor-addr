...
namespace UorAddr1.AddressShape

open UorAddr1.HexEncoding
...
`crates/uor-addr-1/src/resolvers.rs::AddressKInvariantResolver::resolve`. -/
...
`ADDRESS_LABEL_BYTES` in `crates/uor-addr-1/src/model.rs`. -/
...
theorem kappa_suffix_is_hex_of_nibble
  ...
  · refine ⟨digest ⟨((i : Nat) - 7) / 2, by omega⟩ >>> 4, ?_, ?_⟩
    · exact UorAddr1.HexEncoding.high_nibble_lt_sixteen _
    · simp only [hp, if_true]
  · refine ⟨digest ⟨((i : Nat) - 7) / 2, by omega⟩ &&& 0x0F, ?_, ?_⟩
    · exact UorAddr1.HexEncoding.low_nibble_lt_sixteen _
...
theorem address_hex_digits_are_lowercase
  ...
  rw [hk]
  exact UorAddr1.HexEncoding.hex_lower_in_lowercase_hex_alphabet n hn

end UorAddr1.AddressShape
